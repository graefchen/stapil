...
class ISteamNews {
  ...
  static async GetNewsForApp(
    ...
    const link =
      `http://api.steampowered.com/ISteamNews/GetNewsForApp/v1/?appid=${appid}&maxlength=${maxlength}&enddate=${enddate}&count=${count}&tags=${tags}`;
  ...
  static async GetNewsForApp_2(
    ...
    const link =
      `http://api.steampowered.com/ISteamNews/GetNewsForApp/v2/?appid=${appid}&maxlength=${maxlength}&enddate=${enddate}&count=${count}&feeds=${feeds}&tags=${tags}`;
    const response = await SteamWebApiRequest(link);
    return response;
  }
}
