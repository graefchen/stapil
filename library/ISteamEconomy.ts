class ISteamEconomy {
	 ...
	 */
	static async GetAssetClassInfo(key: string, appid: number, language = "en", class_count = 1, classid = [], instanceid = [])
	{
		...
		if (!Array.isArray(classid)) throw new Error("ClassID needs to be an array.");
		if (!Array.isArray(instanceid)) throw new Error("InstanceID needs to be an array.");
		let link = `http://api.steampowered.com/ISteamEconomy/GetAssetClassInfo/v1/?key=${key}&appid=${appid}&language=${language}&class_count=${class_count}`;
	 ...
	 * @param {string} language - The user's local language
	 */
	static async GetAssetPrices(key: string, appid: number, currency = "", language = "en")
	{
		if (key == undefined) throw new Error("Key needs to be given as an parameter.");
		if (appid == undefined) throw new Error("AppID needs to be given as an parameter.");
		const link = `http://api.steampowered.com/ISteamEconomy/GetAssetPrices/v1/?key=${key}&appid=${appid}&currency=${currency}&language=${language}`;
		const res = await fetch(link);
		const data = await res.json();
		return data.response != undefined ? data.response : data;
	}
